...
def get_db_cli_params(url: str) -> tuple[str, dict[str, str]]:
    """Convert testcontainers URL to CLI-compatible format with env.

    Returns:
        Tuple of (cli_url, env_dict) for use with runner.invoke().
    """
    cli_url, password = parse_db_credentials(url)
    return cli_url, {
...
class TestLoadAnnotationCommand:
    ...
    def test_load_annotation_success(self, tmp_path, postgres_container):
        ...
        cli_url, env = get_db_cli_params(postgres_container.get_connection_url())
        result = runner.invoke(
            app,
            [
                "load-annotation",
