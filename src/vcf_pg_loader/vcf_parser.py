...
class VCFHeaderParser:
    ...
    def __init__(
        self,
        sanitizer: VCFHeaderSanitizer | None = None,
        ...
        self._sanitization_result: SanitizedHeader | None = None
    ...
    def sanitization_result(self) -> SanitizedHeader | None:
        ...
        return self._sanitization_result
    ...
    def parse_from_vcf(self, vcf) -> None:
        ...
        if self._sanitize_headers and self._sanitizer:
            raw_header = vcf.raw_header
            self._sanitization_result = self._sanitizer.sanitize_header(raw_header)
            ...
            if info.get("HeaderType") == "CONTIG":
                ...
                if contig_id:
                    self._contigs[contig_id] = {
                        k: v for k, v in info.items() if k not in ("HeaderType", "ID")
                    }
    ...
    def parse_info_fields(self, header_lines: list[str]) -> dict[str, dict[str, str]]:
        ...
        for line in header_lines:
            ...
            if match:
                ...
                if field_def:
                    info_fields[field_def["ID"]] = {k: v for k, v in field_def.items() if k != "ID"}
...
class VariantParser:
    ...
    def parse_variant(
        ...
        for alt_idx, alt in enumerate(variant.ALT):
            ...
            if self.normalize:
                norm_pos, norm_ref, norm_alts = normalize_variant(chrom, pos, ref, [current_alt])
            ...
            record = VariantRecord(
                ...
                if variant.FILTER and variant.FILTER != "."
                else [],
...
class VCFStreamingParser:
    ...
    def get_sanitization_report(self, source_file: str) -> SanitizationReport | None:
        ...
        result = self.sanitization_result
        if result is None:
            return None
        from .phi.header_sanitizer import PHIScanner

        scanner = PHIScanner()
        risk_level = scanner._calculate_risk_level(
            [{"type": item.pattern_matched} for item in result.removed_items]
        )
        return SanitizationReport(
            load_batch_id=None,
            source_file=source_file,
            items_sanitized=len(result.removed_items),
            summary=result.summary,
            phi_detected=result.phi_detected,
            risk_level=risk_level,
            sanitized_items=result.removed_items,
        )
